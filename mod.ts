...
export class Color {
  ...
  static cmyk(c: number, m: number, y: number, k: number) {
    return new Color([Math.round(c), Math.round(m), Math.round(y), Math.round(k)], 'cmyk')
  }
  ...
  negate() {
    const rgb = this.rgb()
    const negated = Color.rgb(255 - rgb.channels[0], 255 - rgb.channels[1], 255 - rgb.channels[2])
